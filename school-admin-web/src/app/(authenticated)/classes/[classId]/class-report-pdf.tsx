...
function ClassReportDocument({ report, schoolName, levelsEnabled }: { report: ClassReport; schoolName?: string; levelsEnabled?: boolean }) {
  return (
...
export async function downloadClassReportPdf(report: ClassReport, schoolName?: string, levelsEnabled = true): Promise<void> {
  const blob = await pdf(<ClassReportDocument report={report} schoolName={schoolName} levelsEnabled={levelsEnabled} />).toBlob();
