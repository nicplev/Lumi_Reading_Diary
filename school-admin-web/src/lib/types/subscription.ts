...
/** Pricing tier keyed by school size (student count). */
export type SubscriptionTier = 'S' | 'M' | 'L' | 'XL';

export interface SubscriptionTierBand {
  tier: SubscriptionTier;
  /** Inclusive lower bound of the student-count band. */
  minStudents: number;
...
/** Derive the tier band a student count falls into. */
...
export interface AcademicYearConfig {
  /** The year currently in session (calendar year the AU school-year starts). */
