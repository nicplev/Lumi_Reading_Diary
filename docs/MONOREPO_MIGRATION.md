...
- **2026-05-20** — Login still broken after the allowlist fix: every server route 500'd. Root cause took a long diagnosis chain — `/api/auth` returned bare `Internal Server Error`; Cloud Run logs showed `ERR_MODULE_NOT_FOUND: firebase-admin-<hash>` (Turbopack hash-aliasing the externalized firebase-admin). Dead ends ruled out along the way: `serverExternalPackages` (no effect); removing `@lumi/server-ops`'s runtime firebase-admin import — swapped `FieldValue.serverTimestamp()` for `new Date()`, commit `bd52270`, kept as a genuine improvement but not the fix; converting `next.config.ts`→`next.config.js` CJS (commit `e863cd5`, real fix for a separate "Unrecognized key(s): __esModule,default" issue where firebase-tools loaded the TS config unwrapped — keep it). **Actual root cause:** firebase-tools' web-frameworks adapter supports Next.js 12–16.0 (stated in every deploy log); admin was on 16.1.6. The 16.1 Turbopack build + their Cloud Build pipeline emit a broken external-module specifier. Fix: pin `next` + `eslint-config-next` to `16.0.11` (commit `559bf20`). After that, `/api/auth` returns proper JSON (`401` for a bad token) and the SSR function loads firebase-admin. **Login works.** FOLLOW-UP: revisit Next 16.1+ only once firebase-tools updates its supported range.
- **2026-05-20** — Post-deploy auth fix. First login attempt on the live site failed with `auth/requests-from-referer-https://lumi-dev-admin.web.app-are-blocked`. Two separate `lumi-kakakids` allowlists didn't know about the new site domain: (1) the **Browser API key** HTTP-referrer restriction (`gcloud services api-keys update` — added `https://lumi-dev-admin.web.app/*` + `.firebaseapp.com/*`); (2) **Firebase Auth authorized domains** (Identity Toolkit admin API PATCH — added `lumi-dev-admin.web.app` + `.firebaseapp.com`). Both edits were additive. NOTE FOR A FUTURE PROD SITE: any new Hosting site needs the same two allowlist additions before its login page will work.
- **2026-05-16** — Phase 6.2 closed. **admin portal live at https://lumi-dev-admin.web.app** (HTTP 200, real SSR `<title>Lumi Admin</title>` page). Took 9 deploy rounds to get there; every failure was a genuine environmental gap rather than a workflow bug, so the workflow file ends up being a clean reference for any future re-deploy. Fixes applied along the way: (1) drop `version:` from `pnpm/action-setup` so it reads `packageManager` from root `package.json`; (2) rewrite `admin/package.json` workspace:* deps to file: at deploy time so firebase-tools' internal npm install resolves them; (3) `NPM_CONFIG_LEGACY_PEER_DEPS=true` on the deploy step env for the firebase-tools npm shell (tremor wants react@18, we have 19); (4) strip the top-level `functions` block from `firebase.json` on the CI copy so the SSR Cloud Run deploy doesn't inherit `functions/`'s lint/build predeploy chain; (5) enable `run.googleapis.com`, `eventarc.googleapis.com`, `cloudbilling.googleapis.com` APIs on `lumi-kakakids`; (6) grant the deploy SA `roles/cloudfunctions.admin` (for `cloudfunctions.functions.setIamPolicy` on the SSR function), `roles/compute.viewer` (for the new compute-SA lookup), and `roles/serviceusage.serviceUsageConsumer`; (7) write `admin/.npmrc` with `legacy-peer-deps=true` so Cloud Build's container `npm install` inherits the setting (the GH-runner env var doesn't propagate to Cloud Build); (8) post-deploy, grant `allUsers` `roles/run.invoker` on the `ssrlumidevadmin` Cloud Run service — firebase-tools doesn't add public-invoke automatically. All future merges to `main` touching `admin/**`, `packages/**`, or workspace files will redeploy via `admin-deploy.yml` in ~2-5 min (provisioning is now done; subsequent deploys skip the first-time Cloud Run/Artifact Registry setup). `workflow_dispatch` also wired in. Outstanding: move `FIREBASE_SERVICE_ACCOUNT_KEY` out of `.env.production` into Firebase Functions Secret Manager — tracked as a `followups` item.
...
- **2026-05-16** — Branch cleanup done. `monorepo-migration` (last sha `5a952db`) deleted on `origin` and locally — `git branch --merged main` confirmed it was fully merged into `main` before deletion. Rollback path is now the `pre-monorepo-merge` tag (both repos) and the `post-monorepo-merge` tag on `main`; no migration-specific branch is needed any longer. Only Phase 6.2 (deploy) remains in `STATUS.followups`.
