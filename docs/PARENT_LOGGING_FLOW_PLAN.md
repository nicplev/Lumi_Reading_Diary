...
**Status:** IMPLEMENTED v4 — all phases merged 2026-07-24, incl. the super-admin Operations toggle for `parentBackdating` (D1 condition ✅ — no longer owed). **PENDING: prod deploys (rules + functions, Nic confirm; admin-portal deploy rides CI) · app release.**
**Date:** 2026-07-24
...
| P2-3 | `editedAt` on the model (read + local cache; NEVER in `toFirestore` — create rules forbid it) with `Edited h:mm a` display in the review sheet + session detail; detailed success screen gains `Edit this log` / `Remove my session` beside the tappable `Done — returning home…` (celebration never blocks recovery); audience disclosure `Notes and recordings are shared with {child}'s teacher.` in the detailed flow's optional-detail step + the quick-log follow-up | ✅ merged 2026-07-24; suite 647/647. **Phase 2 complete.** |
...
- **2026-07-24** — v3: ALL PHASES MERGED in one run. Phase 1: #555 (rules) / #558 (functions) / #560 (client foundation) / #564 (Home rows) / #565 (review+edit/remove) / #567 (offline UX) / #568 (teacher context). Phase 2: #570 (backdating + guardian prefs) / #572 (picker + books[]) / #573 (provenance + success recovery + disclosure). Phase 3: #574 (analytics; remaining items deferred with rationale). Suites: flutter 647/647, functions 260 unit + 9 integration, rules 196 + 20 slot cases. **Outstanding:** ① `firebase deploy --only firestore:rules` then `--only functions` (Nic confirm; rules MUST precede the app release — new clients write occurredOn/context/books/slot docs prod rules would reject); ② app release train; ③ ~~Nic: super-admin Operations toggle for `platformConfig/parentBackdating`~~ ✅ BUILT (same run): Operations → Feature Controls → "Parent backdating — Yesterday (platform)" — fails-open card in the coverOcr idiom (`configured` disclosure, reason required on disable, audit events `parent_backdating_enabled/disabled`, resolver parity with the app's `isParentBackdatingEnabled` pinned by `packages/server-ops/test/parentBackdating.test.ts`); admin portal deploys via CI on merge; ④ post-adoption rules-hardening PR (require occurredOn/context) once old clients are gone; ⑤ D4 note: `parentQuickLogV2` client kill-switch was superseded — the old row widget was fully replaced in PR-D rather than kept behind a flag (rollback = app-store rollback; flag would have doubled every widget test for one release of insurance).
