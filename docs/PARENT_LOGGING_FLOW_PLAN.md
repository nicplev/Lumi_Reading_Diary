...
| E | Today's sessions sheet + owner-scoped Edit screen + Remove with last-session warning; success-screen relabel (`Edit this log` vs `Add another session`) | new `today_sessions_sheet.dart`, `edit_reading_log_screen.dart`, `session_detail_sheet.dart`, `reading_success_screen.dart` | app release | ☐ |
| F | Offline UX: pending row state, Edit/Cancel pending, reconnect conflict prompt, Offline & Sync surfacing | `offline_service.dart`, `offline_management_screen.dart`, row widget | app release | ☐ |
| G | Teacher context: home/classroom toggle on the teacher log sheet; classroom rows on parent Home; teacher home-proxy claims slot | `teacher_log_reading_sheet.dart`, row widget | app release | ☐ |
