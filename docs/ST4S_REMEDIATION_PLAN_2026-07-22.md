...
| **PF51** — file uploads follow OWASP incl. AV scanning | "Yes – majority incl. AV scanning" — Ready (low risk) | Upload validation is strong (type/size/path rules) but there is **no antivirus/malware scanning** of uploaded files. The roster-import upload path was separately reviewed for injection on 24 Jul (#581) — two stored-injection findings fixed; that review does **not** change the AV position. | Either add scanning later or answer "majority, excluding AV" next time. Decide at Phase 7 |
...
| 2026-07-24 | GO1/GO2 · D3 · HR1/HR2 | Sign-and-fill artifacts drafted: GO1/GO2 appointment letters, restore-drill record (D3), training record (HR2). Nic holds a current WWCC → HR1 met in substance (record details privately). | — | ◐ |
| 2026-07-24 | Q5 / PF51 / EV13 | **Security review of the new SIS/CSV roster-import upload path** (OWASP-aligned, scoped to "can this upload be used to take advantage of the system"). Two **stored-injection** findings, both fixed. **(1) CSV/formula injection (medium):** three exports re-emitted stored names as spreadsheets while neutralising none of `= + - @` TAB CR — class report, comprehension eval, and the **staff credentials export, whose adjacent column is a temporary password**, so a formula smuggled in via an imported name could exfiltrate that password on open. Fixed at the sink (shared `csvCell()`/`toCsv()` in `school-admin-web/src/lib/csv-export.ts`), which also covers hand-typed and app-sourced names. **(2) HTML injection (medium):** `functions/src/email_templates.ts` spliced student/staff/school names and login email into parent- and staff-facing email HTML unescaped, while `escapeHtml()` sat in the same file — a roster import could plant phishing markup delivered from the school's own trusted sender. Escaped at the leaf renderers. Plus `parentEmail` format-validation parity across all 3 import routes and field-length caps (were row-count-capped only). **Verified clean, no change needed:** no IDOR/tenant escape (doc ids resolve only under the server session's `schoolId`; every non-create action requires `snap.exists`), authz fails closed on impersonation/demo/no-MFA, no prototype pollution, no ReDoS, no portal XSS, `.xlsx` parsed client-side so a zip bomb can't reach the server. Regression tests: `scripts/csv-export.test.ts` (8, asserts the real HYPERLINK/WEBSERVICE/DDE payloads) + 5 email-escaping tests. Full write-up: **VAR Addendum A** (`docs/security/VULNERABILITY_ASSESSMENT_REPORT_2026-07-24.md` §11, findings F-10/F-11). **Deployed to prod 24 Jul** (functions + school portal). | #581 | ☑ |
| 2026-07-24 | Q5 / PF51 / EV13 | **F-10 preventive control + 2 further sinks.** Added `scripts/check-csv-exports.sh`: (a) every approved CSV encoder must still neutralise formulas — so the apostrophe prefix can't be "simplified" out while call sites keep passing; (b) every file emitting a `text/csv` payload must use an approved encoder or carry a documented `csv-export-guardrail:` exemption. Runs in demo-readiness CI **and** as a `firebase.json` predeploy on both portals, so neither a PR nor a manual deploy can bypass it; proven to fail in both directions before being relied on. **On first run it found two more live F-10 sinks in the super-admin portal** that the manual review had not reached — `toCsvString` (backs the super-admin student, reading-log and allocations exports, carrying the same imported names) and the impersonation-audit export. Fixed at the encoder. Deployed: school portal + functions manually, super-admin portal via `admin-deploy` CI. | #585 | ☑ |
