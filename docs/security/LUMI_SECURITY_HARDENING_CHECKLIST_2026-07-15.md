...
- **ADC replacement status:** User `gcloud` login exists as `nicplevrit7@gmail.com`; the CLI default project was corrected from the retired `lumi-kakakids` to `lumi-ninc-au`, but Application Default Credentials are absent. Before disabling/deleting `ade3…e687`, run `gcloud auth application-default login` and `gcloud auth application-default set-quota-project lumi-ninc-au`, verify a dry-run demo/admin operation through ADC, disable the key, repeat production/portal smoke tests, then delete the key and local JSON after a short successful soak.
