...
- **Resume point if interrupted:** inspect the latest `git diff`; continue at P1-7. Do not rebuild the account/student deletion or audio emulator suites. Audio authorization, canonical-path selection, kill-switch behavior, upload confirmation, deletion and retention are locally covered. Remaining audio work is: enable/prove App Check in staging, prove real Cloud IAM URL signing, test microphone/upload/playback on physical iOS/Android devices, and either add a real media decode/transcode service or formally accept/disable the header-only validator. P0-7 needs a canary deployment.
...
- **Where:** as above.
- **Fix:**
  - [ ] Default child-related Analytics + voice collection **off**; document necessity; obtain school/parent authority; support withdrawal.
  - [ ] Remove direct UID linkage + detailed child attributes unless demonstrably necessary.
  - [ ] Verify actual Firebase SDK network traffic vs App Store/Play privacy labels (`docs/app-store/app-privacy-labels.md`). If using Apple Kids Category, apply its third-party-analytics restrictions.
  - [ ] Before any AI/LLM analytics: complete a PIA (purpose, fields, training use, retention, subprocessors, offshore access, deletion, AU residency). *(No LLM data transfer found in repo today.)*
- **Verify:**
...
- **Severity:** Medium
- **Why:** Current-tree scan found no private service key, but Git history + build artifacts weren't scanned, and demo/review default passwords remain. Also: `school-admin-web/service-account.json` is a real admin key for the *old* project `lumi-kakakids` — gitignored + untracked, but on disk and baked into `.next/server`.
- **Where:** `scripts/seed_demo_school.js:151` + review-account script; `school-admin-web/service-account.json`; `.dart_define.json` (`GOOGLE_BOOKS_API_KEY` is extractable — restrict or proxy).
- **Fix:**
  - [~] Run a history-aware secret scanner (e.g. Gitleaks) in CI; scan built APK/IPA/web artifacts, not just source. *(Gitleaks 8.30.1 scanned 561 commits after reviewed false-positive fingerprints: clean. A pinned redacted GitHub workflow and reusable `scripts/security/scan-secrets.sh` were added. The release APK, portal server output and portal static output contain zero private keys. An iOS IPA artifact was not available to scan.)*
  - [~] Rotate the `lumi-kakakids` key; move the portal to Application Default Credentials / Secret Manager. *(Portal code now uses ADC only and no longer loads a JSON credential path, preventing key material from entering deploy artifacts. The ignored legacy JSON file still exists locally and its key must be disabled/rotated in Google Cloud, then the file and any old `.firebase`/`.next` caches must be purged.)*
  - [ ] Restrict Firebase browser/iOS/Android API keys by bundle ID/package/signing cert/API/quota in GCP.
- **Verify:**
  - [~] Gitleaks CI green on history + artifacts; portal boots without a JSON key on disk. *(Local history and available deploy-artifact scans are green; portal production build passes with ADC-only code. Await the first GitHub workflow run, IPA scan, cloud key rotation and a runtime smoke test using the deployed service account.)*
...
- `[ ]` Google API-key restrictions, Firebase Auth quotas, SMS abuse alerts.
