...
- **Demo audio test prepared (active handoff):** The target account is the synthetic parent `support+demo.parent@lumi-reading.com` in `schools/lumi_demo_primary_school`; the school is confirmed `isDemo:true`, the parent membership is active with two linked fictional children, and the platform recording kill switch is on. Recording was enabled only for the demo school and both demo classes received the synthetic prompt “Tell us one thing that happened in the story.” A fresh patched/App-Check-enabled APK is installed on the Samsung, the named debug token `Samsung A57 demo audio test 2026-07-16` is registered, activation is visible in device logs, and the app is at the parent login screen awaiting the password. After the audio/offline test, revoke this debug token, clear/uninstall the canary, restore normal USB stay-awake behavior, and decide whether to leave the demo-school recording toggle enabled for future demos.
