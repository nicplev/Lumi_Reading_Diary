...
- **Current item:** finish P1-3 with a release-device network capture and live App Store/Play questionnaire updates, then finish P1-7 with the Play App Signing certificate, store-signed smoke tests and the remaining artifact/quota evidence. P0-1 is complete: production audio validation, class-scoped signing/deletion, staged callable App Check, replay handling, physical iOS parent capture/playback and physical Android teacher playback/deletion are proven. P1-1 still requires App Attest/Play Integrity evidence and the broader staged service rollout after the store accounts are available.
- **Status:** P0-1 through P0-8, P1-2, P1-5, P1-6, P1-8 and P2-2 are complete; P1-1/P1-3/P1-4/P1-7/P2-1 are partially implemented. Optional Analytics and Crashlytics now have native and Dart fail-closed defaults, separate adult controls on the shared parent/staff Account screen, no direct account UID, no detailed child-reading attributes, advertising consent permanently denied, and aligned policy/repository label guidance. Audio uploads now enter a create-only untrusted pending namespace; Storage Rules bind that object to the owning parent/log, and `confirmComprehensionAudioUpload` fully decodes and canonicalises it in an isolated, no-data-permission FFmpeg worker before publishing the server-owned playback object and validation receipt. Rejected generations are removed, stranded pending objects expire after 24 hours, playback signs the exact validated generation, and validation attempts are server-rate-limited. App Check enforcement remains safely off by default behind `COMPREHENSION_AUDIO_APP_CHECK_ENFORCED`; limited-use client tokens and explicit replay rejection are implemented and passed a temporary production canary. Teacher access and client queries are class-scoped, reading-log identity/system fields are immutable, create schemas validate optional values, proxy child/class mismatch is denied, log IDs are random 128-bit values, and comments are schema-checked against their authoritative parent log. Parent memberships can no longer be self-created and public demo enquiries now pass through the App-Check-ready, durably rate-limited `submitDemoRequest` callable; direct client access to `schoolOnboarding` is denied. Mutable portal routes now fail closed if current membership cannot be verified, legacy `/users` writes are terms-only, and mobile dev-access lookup no longer exposes an email-hash oracle. Incremental student/class aggregation is live and fully reconciled; class query batching and local-day removal checks now respect Firestore limits and Australian DST. Firestore now has seven-day PITR and deletion protection, a timed clone/verification drill passed, and an alert-only production budget is active. Dangerous client provisioning and checked-in demo passwords are absent from the release APK. Functions and portal production dependency trees have zero critical/high advisories.
...
- **Verification so far:** `cd functions && npm run test:functions` → **127/127 passed**; `npm run test:rules` → **146/146 passed**; `npm run test:rules:storage` → **13/13 passed**; `npm run test:audio:integration` → **7/7 passed**; `npm run test:audio:http` → **4/4 passed**; `npm run test:audio:appcheck` → **1/1 passed**; `npm run test:deletion:integration` → **2/2 passed**; targeted Flutter reading/offline/audio tests → **47/47 passed**. Real-media tests fully decode a valid M4A and reject both a truncated `ftyp` header and a 62-second file. Functions build and lint pass (8 existing warnings, 0 errors), and the production dependency audit has **0 critical/high** advisories (8 moderate; the remaining `uuid` chains are not called with attacker-controlled input). Earlier targeted Flutter service/screen suite → **50/50 passed**, onboarding/callable migration suite → **28/28 passed**, mobile dev-access suite → **2/2 passed**, portal session-policy suite → **2/2 passed**. The Next 15.5.20 production build succeeds. `flutter analyze --no-fatal-infos` → no errors/warnings (**123 existing info lints**). The broad Flutter run reports **400 passed / 8 unrelated UI-test failures**: stale finders in feelings tracker, splash, teacher assignment card, comment chips and week progress bar, plus a pending toast timer in the awards test. Android release APK builds successfully and provisioning/password/private-key scans are clean. Live API-key negative probes reject an unlisted web referrer, iOS bundle and Android signing certificate while accepting every registered Lumi identity; all three current Firebase keys retain their 27-service API allowlist. `git diff --check` and the redacted Gitleaks working-diff scan are clean.
- **Resume point if interrupted:** inspect the latest `git diff`; finish the P1-3 release-device capture, then obtain the Google Play App Signing SHA-1 before producing a store-signed Android build and finish P1-7's IPA/signed-device/quota evidence. Do not rebuild the account/student deletion or audio validation suites. Audio authorization, pending/canonical isolation, full decode/transcode, generation-bound receipts, kill-switch behavior, deletion, retention, debug-provider App Check and direct-install iOS-parent/Android-teacher UI paths are proven. Remaining audio attestation work is store-signed App Attest/Play Integrity traffic and the subsequent staged enforcement rollout under P1-1. Uninstall the now-inert debug apps when the disconnected phones are next attached; Android app data is cleared, iOS was force-signed-out/cache-cleared, both debug registrations are revoked and the token-bearing local build/log files are gone. The P0-7 refreshed-bundle canary is already deployed.
...
- **Least-privilege boundary:** the worker receives only base64 bytes—no school, child, log, bucket or UID—and runs as `lumi-audio-validator@lumi-ninc-au.iam.gserviceaccount.com`. That identity has zero project IAM roles and zero user-managed keys. Cloud Run invocation is private and bound only to `lumi-ninc-au@appspot.gserviceaccount.com`; an unauthenticated production probe returned HTTP 403.
...
- **IAM audit remains open:** 69 Gen2 functions share the App Engine service account, two web backends use the Compute default service account, and both accounts still have project `Editor`. The GitHub deploy account uses a recently active user-managed key and broad deploy roles. The local Firebase Admin key has now been replaced with ADC, verified, disabled and permanently deleted together with its JSON file. Migrate CI to workload identity and replace shared Editor identities with per-workload least-privilege accounts before checking off IAM.
...
Lumi is still **not ready to hold production children's data** until the remaining controls are completed and proven in the live project. The main release blockers have shifted to store-attested App Check rollout, signed-device analytics/privacy evidence, Play signing, remaining ops evidence, and offline/timezone conflict testing. Account/student deletion still needs a signed-in/on-device smoke test. Audio authorization, full decode/transcode validation and direct-install iOS parent/Android teacher UI paths are production-proven; the remaining device gate is store-signed App Attest/Play Integrity evidence.

| Area | Status | One-line |
|---|---|---|
| Firestore rules | 🟢 repo / 🟢 live | 146 emulator tests pass; active production source hash exactly matches the reviewed local rules. |
...
- `[~]` **P2-1 Budgets + dashboards.** Cloud Billing budgets/alerts, per-service anomaly monitoring, quotas, security/cost dashboards. Alert on reads/writes/egress/functions/SMS/Storage. *(Project-scoped AUD 100 monthly budget is active with 50/90/100% current-spend and 100% forecast alerts to the billing admin. Primary and backup security email channels are attached to five enabled high-signal Cloud Run/Firestore/App Check/Auth/SMS policies. A delivery test plus read/write/egress/Storage anomaly coverage and dashboards remain.)*
...
- `[~]` **P2-5 Device + error-path testing.** Old iPads / low-end Android; offline/DST/date-tamper; account revocation; force-upgrade (make it fail into a support mode when version config is unavailable + configure store URLs); no-network, expired-session, empty-class, denied-camera, book-not-found/manual-entry paths on real devices. *(Samsung A57/Android 16 direct-install baseline builds and launches without a Lumi fatal exception. Its reinstall exposed and verified the P1-9 backup/device-transfer fix. Interactive synthetic-account, offline/audio/consent/error-path runs and genuinely old/low-end hardware remain.)*
...
- `[x]` Deployed `firestore.rules` + `storage.rules` **exactly match** the reviewed source. *(Rules API source hashes: Firestore local/remote `2698760b…502824`; Storage local/remote `0f5e2464…b347`, active ruleset `1ca9ac53-19c6-4fe2-b552-7fa056069f5f`.)*
- `[~]` App Check registration + metrics + enforcement (Firestore, Storage, Auth, Functions). *(Metrics were audited, but all observed traffic is invalid/missing/unknown-app and Firestore/Storage/Auth remain correctly unenforced. Enforcing before a valid signed-device/web canary would cause an outage.)*
- `[ ]` Firestore/Storage/Auth/Functions resource locations + cross-border support/data flows are AU.
- `[~]` Least-privilege IAM + service-account permissions. *(Both portals use ADC and two exposed/duplicated Admin keys were revoked. Shared App Engine/Compute `Editor` runtimes, a user-managed CI deploy key and one unowned Firebase Admin key remain.)*
...
- `[~]` Cloud Billing budgets/alerts + per-service anomaly monitoring. *(AUD 100 project-only monthly alert budget is live; build per-service anomaly policies next.)*
