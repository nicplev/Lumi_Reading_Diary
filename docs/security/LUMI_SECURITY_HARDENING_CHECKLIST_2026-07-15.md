...
| Cloud Storage | 🟢 rules / 🟠 rollout | Pending/canonical isolation and full decode are live; staged App Check plus physical iOS/signed teacher-UI evidence remain. |
...
- [x] **Audio read is denied** (`storage.rules`) and the app fetches via the class-authorised `getComprehensionAudioUrl` callable, which derives the canonical path and signs only its validated generation. *(Local integration and production signer IAM are proven; staged production App Check remains under P0-1.)*
...
- **Fix:**
  - [~] Activate App Check in release builds; watch metrics; then enforce on Firestore, Storage, Auth and every callable that supports it. Make the release build fail if the flag is omitted. *(A physical Android debug-provider canary is registered and now produces `VALID/ALLOW` Firestore/Auth metrics. Release fail-closed configuration, Play Integrity/App Attest and enforcement remain.)*
...
- **Verify:**
  ...
  - [~] Console: App Check enforced for Firestore/Storage/Auth/Functions; enforcement metrics healthy. *(Android debug canary metrics are valid, but all services intentionally remain unenforced until store-signed clients exist and older traffic is migrated.)*
