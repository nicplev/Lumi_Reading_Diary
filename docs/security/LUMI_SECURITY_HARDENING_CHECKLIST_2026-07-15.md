...
- **Super-admin moved to ADC:** `admin/src/lib/firebase-admin.ts` and the admin deployment workflow no longer accept or write `FIREBASE_SERVICE_ACCOUNT_KEY`. Cloud Run revision `ssrlumidevadminau-00059-mux` uses its attached identity. A real custom-token login returned 200, the server verified `/superAdmins`, and `/api/dashboard` returned 200 with its full payload. The duplicated current-project key `9dd4…884c` was then disabled, deleted and removed from local files; the same authenticated dashboard call still returned 200 after revocation. A no-ignore fingerprint scan found no copy in the workspace/key directory. Redacted artifact triage found only the public Firebase browser identifier and Next's generated server-side preview/action signing keys in protected server manifests—not the revoked Admin credential.
