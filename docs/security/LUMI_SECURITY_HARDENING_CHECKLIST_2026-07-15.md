...
- **IAM audit remains open:** 69 Gen2 functions share the App Engine service account, two web backends use the Compute default service account, and both accounts still have project `Editor`. The GitHub deploy account uses a recently active user-managed key and broad deploy roles; one second Firebase Admin user-managed key (`ade3…e687`) has no local fingerprint or 90-day audit-log match but was not revoked because another machine may own it. Migrate CI to workload identity, inventory that remaining key and replace shared Editor identities with per-workload least-privilege accounts before checking off IAM.
...
- `[~]` **P2-1 Budgets + dashboards.** Cloud Billing budgets/alerts, per-service anomaly monitoring, quotas, security/cost dashboards. Alert on reads/writes/egress/functions/SMS/Storage. *(Project-scoped AUD 100 monthly budget is active with 50/90/100% current-spend and 100% forecast alerts to the billing admin. Per-service anomaly policies/dashboards remain.)*
