...
| Secret Manager | SendGrid credentials and portal session secret | Existing secrets use automatic multi-region global replication | No child content intended | Narrow secret-level access; no client exposure | Decide whether to migrate to new AU user-managed-replication secrets | Conditional |
